...
%created by macshen
function [y,y_det,c_err,c_std]=cmm_smoothed_static(mp_flag)
...
for i = 1:Ns  %draw the first step
   ...
   % pr_err=[pr_err;pr_err_vec(1)]; %added by macshen
    ...
    posi=[pf(:,1)+estenu{1}(i,1),pf(:,2)+estenu{1}(i,2)];%estimated positions
    for k=1:Np
    posi_xyz(:,k)=enu2xyz([posi(k,:),0],orgxyz);
    pr_es(k)=norm(posi_xyz(:,k)-svxyzmat{1}(1,:)');
    end
    true_pr=norm(usrxyz{1}-svxyzmat{1}(1,:)');
    es_com_err(i)=mean(pr_es-true_pr);
    es_com_std(i)=sqrt(var(pr_es-true_pr));
